...
/**
 * Match fixed patterns in the same violation type.
 * 
 * @author kui.liu
 *
 */
	...
	public void match() throws IOException {
		...
		for (File file : violationTypes) {
			if (file.isDirectory()) {
				...
				String fixedViolationPatterns = filePath + "/FixPatterns/";
				...
				for (File fixedviolationTypeFolder : fixedviolationTypeFolders) {
					if (fixedviolationTypeFolder.isDirectory()) {
						String fileName = fixedviolationTypeFolder.getPath() + "/PatternFeatures.list";
						List<Double[]> features = readFeatures(fileName);
						fixedViolationFeatures.put(fixedviolationTypeFolder.getName(), features);
					}
				}
				...
				for (File centroidFile : centroidFiles) {
					...
					if (fixedViolationFeatures.containsKey(violationType)) {
						...
						String outputFileName = centroidFile.getPath().replace("RQ3", "RQ3_1").replace("ClusterCentroids", "MatchedFixPattern1").replace("Centroid", "FixPattern");
					...
					String outputFileName = centroidFile.getPath().replace("RQ3", "RQ3_1").replace("ClusterCentroids", "MatchedFixPattern2").replace("Centroid", "FixPattern");
					FileHelper.outputToFile(outputFileName, matchedFixPattern, false);
				}
			}
