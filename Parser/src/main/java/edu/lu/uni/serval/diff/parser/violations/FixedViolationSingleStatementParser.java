...
/**
 * Parse diffs of fixed violations with GumTree in terms of single statement.
 * 
 * @author kui.liu
 *
 */
public class FixedViolationSingleStatementParser extends FixedViolationParser {
	...
	public void parseCodeChangeDiffs(File prevFile, File revFile, File diffentryFile) {
		...
		if (actionSets.size() > 0) {
			...
			List<ViolationInstance> violations = readViolations(prevFile, revFile);
			for (HierarchicalActionSet actionSet : actionSets) {
				...
				ViolationInstance violation = findViolation(startLine, endLine, violations);
...
//				this.originalTree += Configuration.BUGGY_TREE_TOKEN + "\n" + actionSet.getOriginalTree().toString() + "\n";
	...
	protected ViolationInstance findViolation(int startLine, int endLine, List<ViolationInstance> violations) {
