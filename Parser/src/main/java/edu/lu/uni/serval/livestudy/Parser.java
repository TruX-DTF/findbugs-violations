...
public class Parser {
	...
	public Parser(Violation violation, File sourceCodeFile) {
	...
	public void parse() {
		int startLine = this.violation.getStartLine();
		int endLine = this.violation.getEndLine();
		
		String violationType = this.violation.getViolationType();
		...
		if ("EQ_DOESNT_OVERRIDE_EQUALS".equals(violationType)|| "HE_EQUALS_USE_HASHCODE".equals(violationType)
			...
			if (classNameTree != null) {
				...
				System.err.println("#Null_Violation_Hunk: " + this.violation.getViolationType() + ":" + this.sourceCodeFile.getPath() + ":" + startLine + ":" + endLine);
			}
		} else {
			treeParser.extract();
