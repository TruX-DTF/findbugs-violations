...
public class ParseViolationWorker extends UntypedActor {
	private static Logger log = LoggerFactory.getLogger(ParseViolationActor.class);
	
	private String sourceCodeFilesPath;
	
	public ParseViolationWorker(String sourceCodeFilesPath) {
		this.sourceCodeFilesPath = sourceCodeFilesPath;
	}

	public static Props props(final String sourceCodeFilesPath) {
		return Props.create(new Creator<ParseViolationWorker>() {

			private static final long serialVersionUID = -7615153844097275009L;

			@Override
			public ParseViolationWorker create() throws Exception {
				return new ParseViolationWorker(sourceCodeFilesPath);
			}
